...
export default function App() {
  ...
  // Global helper for manual session clearing (can be called from browser console)
  useEffect(() => {
      ...
      try {
        ...
        if (session && !error) {
          ...
          if (!session.access_token.startsWith('eyJ')) {
            console.error('[Startup] 🚨 Invalid token format detected - clearing immediately');
            await supabase.auth.signOut();
            localStorage.clear();
            return;
          }
          ...
          if (refreshError && (
            ...
            console.error('[Startup] 🚨 Invalid session detected - clearing immediately');
            console.error('[Startup] Error:', refreshError.message);
            await supabase.auth.signOut();
            localStorage.clear();
            
            toast.error('Session Invalid', {
              description: 'Your previous session was invalid and has been cleared. Please log in.',
              duration: 5000,
            });
          }
        }
